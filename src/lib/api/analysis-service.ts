...
export async function startAnalysis(query: string, userId: string = 'system') {
  ...
  try {
    ...
    if (finalState.errors?.length > 0 && finalState.humanReviewStatus !== 'Pending') {
      ...
      if (criticalFailure) {
         ...
         return { analysisId: analysis.id, status: AnalysisStatus.FAILED };
      }
    } 
  ...
  const finalAnalysis = await getAnalysisProgress(analysis.id);

  return {
    analysisId: analysis.id,
    status: finalAnalysis?.status || AnalysisStatus.COMPLETED,
    estimatedSteps: 11,
    graphVersion: "1.0.0"
  };
}
